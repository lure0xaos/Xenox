...
class Field(private val resources: Resources) {
    ...
    fun gameStep(game: JGame): Boolean {
        ...
        if (!lostLife) {
            ...
            if (item != null && item!!.position == player!!.position) {
                ...
                item = null
                if (gotItem(
                        Item.Companion.I_MINUS_BALL,
                        Item.Companion.I_PLUS_BALL,
                        Item.Companion.I_SCORE,
                        Item.Companion.I_LIFE
                    )
                ) {
                    got = true
                }
            ...
            if (got) {
                if (game.options.isSound) {
                    game.soundPlay(
                        resources.load(
                            AudioClip::class.java,
                            *resources.urls(true, Xenox.Companion.SND_GOT)
                        )!!
                    )
                }
            }
        ...
        if (!lostLife) {
            if (item == null) {
                if (rnd.nextInt(Xenox.Companion.ITEM_SHOW) == 0) {
                    item = Item()
                    val pos = Point()
                    do {
                ...
                if (rnd.nextInt(Xenox.Companion.ITEM_HIDE) == 0) {
                    item = null
                }
            }
        }
    ...
    private fun gotItem(vararg types: Byte): Boolean {
        if (gotItem != null) {
            for (type in types) {
                if (type == gotItem!!.type) {
                    when (type) {
                        Item.Companion.I_LIFE -> player!!.lives=(player!!.lives + 1)
                        Item.Companion.I_SCORE -> player!!.score=(player!!.score + 100)
                        Item.Companion.I_MINUS_BALL -> balls.removeAt(
                            rnd.nextInt(
                                balls.size
                            )
                        )
    ...
    fun hasGotItem(last: Boolean, vararg types: Byte): Boolean {
        if (gotItem != null) {
            for (type in types) {
                if (type == gotItem!!.type) {
                    if (last) {
    ...
    fun init(levelNum: Int, level: Level?) {
